#!/usr/bin/env python3
"""
Score Jobs from Top to Bottom - Fixed for current column structure
Handles rows with 10 columns (A-J) and adds K, L, M as needed.
"""
...
BATCH_SIZE = 6  # Smaller batches to avoid rate limits
DELAY_SEC = 10  # Longer delay between batches
...
def get_new_jobs_from_top(limit=30):
    """Get 'New' jobs starting from top (row 2) - handles variable column counts"""
    print(f"📥 Fetching jobs from top (max {limit})...")
    
    # Get all data with more columns than needed
    data = gog(['sheets', 'get', SHEET_ID, 'Sheet1!A:Z',
...
def update_scores(scores):
    """Update sheet with scores - handles missing columns"""
    if not scores:
        return
    ...
    for score_data in scores:
        ...
        # We need to ensure we have all 3 columns
...
def main():
    ...
    print("=" * 60)
    print("📌 Strategy: Score from row 2 downward (newest first)")
    print("📌 Batch size: 6 jobs (to avoid rate limits)")
    print("📌 Delay: 10s between batches")
    print("=" * 60)
    
    # Get new jobs from top
    new_jobs = get_new_jobs_from_top(limit=30)  # Start with 30 jobs
    ...
    print(f"  First: Row {new_jobs[0]['row']} - {new_jobs[0]['title'][:40]}...")
    print(f"  Last:  Row {new_jobs[-1]['row']} - {new_jobs[-1]['title'][:40]}...")
    print(f"  Order: Most recent first (top to bottom) ✅")
    print()
    ...
    for i in range(0, len(new_jobs), BATCH_SIZE):
        ...
        # Show jobs in this batch
        for job in batch:
            print(f"   - Row {job['row']}: {job['title'][:40]}...")
        ...
        if scores:
            ...
            print(f"   ⚠️  Batch failed to score")
    ...
    print(f"   Total 'New' jobs in this run: {len(new_jobs)}")
    ...
    if total_scored > 0:
        print("🎯 NEXT STEPS:")
        print(f"   1. Check rows {new_jobs[0]['row']} to {new_jobs[total_scored-1]['row']} for scores")
        print(f"   2. Run again to score next batch of 'New' jobs")
        print(f"   3. Review high-scoring jobs (≥80) for applications")
    else:
        print("⚠️  No jobs were scored. Check API key and rate limits.")
